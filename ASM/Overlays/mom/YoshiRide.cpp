...
namespace
{
	...
	void ChangeCharacter(Player& player, unsigned character)
	{
		...
		player.bodyModels[player.GetBodyModelID(character, false)]->Copy(*player.bodyModels[player.GetBodyModelID(player.prevHatChar, false)],
...
void YoshiRide::UpdateModelTransform()
{
	...
	{
		rigMdl.mat4x3 = rigMdl.mat4x3.RotationY(ang.y);
...
int YoshiRide::InitResources()
{
	...
	Animation::LoadFile(*headAnimFile);
...
void YoshiRide::EndRide(bool changeChar)
{
	...
	headMdl->~Model();
	delete headMdl;
	headMdl = new ModelAnim;
	headMdl->SetFile(rider->headModels[Player::CH_YOSHI]->data.modelFile, 1, -1);
	((ModelAnim*)headMdl)->SetAnim(headAnimFile->filePtr, Animation::NO_LOOP, 0x1000_f, 0);
	pos = rider->pos;
	rider->pos = rigMdl.mat4x3.c3 << 3;
