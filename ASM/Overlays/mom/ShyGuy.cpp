...
ShyGuy* ShyGuy::Spawn()
{
	...
	static_assert(sizeof(ShadowModel) == 0x28, "Shadow is wrong size");
...
void ShyGuy::UpdateModelTransform()
{
	...
	modelMat = modelMat.RotationY(ang.y);
	modelMat.c3 = pos >> 3;
...
Player* ShyGuy::PlayerVisibleToThis(Player* player)
{
	...
	MATRIX_SCRATCH_PAPER = MATRIX_SCRATCH_PAPER.RotationY(ang.y);
...
{
	if(rigMdl.file != animFiles[WALK].filePtr && rigMdl.file != animFiles[RUN].filePtr)
		return;
...
int ShyGuy::Behavior()
{
	...
	matChg.currFrame.ApproachLinear(0_f, -0x1000_f);
